...
function factory(Task, scheduler, registry, eventsProtocol, schedulerProtocol,
    ...
    TaskGraph.prototype.validate = function () {
        ...
        _.forEach(self.definition.tasks, function(taskData) {
            ...
            if (_.has(taskData, 'taskDefinition')) {
                task = taskData.taskDefinition;
            } else {
                task = registry.fetchTask(taskData.taskName).definition;
            }
    ...
    TaskGraph.prototype._validateOptions = function(taskDefinition, label) {
        ...
        _.forEach(baseTask.requiredOptions, function(k) {
            ...
            if (!option && self.definition.options.defaults) {
                option = self.definition.options.defaults[k];
                if (label && self.definition.options[label]) {
                    option = self.definition.options[label][k] || option;
                }
            }
    ...
    TaskGraph.prototype.start = function () {
        ...
        return self.persist().then(function (record) {
    ...
    TaskGraph.prototype.persist = function persist() {
        ...
        return Q.resolve(
            waterline.graphobjects.findOne({ instanceId: this.instanceId })
            .then(function(doc) {
                if (doc) {
                    return waterline.graphobjects.update(
                        { instanceId: self.instanceId }, self.serialize());
                } else {
                    return waterline.graphobjects.create(self.serialize());
                }
            })
        )
        .catch(function(error) {
