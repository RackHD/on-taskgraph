...
function SubscriberFactory(tgrProtocol, registry, assert, Q, _) {
    ...
    Subscriber.prototype.runTaskGraph = function runTaskGraph(uniqueName, options) {
        var taskGraph = registry.fetchGraph(uniqueName).create(options);
        registry.putActiveGraph(taskGraph.instanceId, taskGraph);
        taskGraph.start();
        return taskGraph.instanceId;
    };

    Subscriber.prototype.cancelTaskGraph = function cancelTaskGraph(instanceId) {
        var instance = registry.fetchActiveGraph(instanceId);
        if (instance) {
            instance.cancel();
        }
    };
