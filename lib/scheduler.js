 ...
 * @returns {Scheduler}
 */
function factory(Logger, assert, util, uuid, Q, _) {
     ...
     * @returns {*}
     */
    Scheduler.prototype.wrapData = function (task, args, schedulerOverrides) {
        assert.ok(task);

        var defaults = {
            id: task.instanceId || uuid.v4(),
        ...
        itemToSchedule.result =  itemToSchedule._result.promise;
        itemToSchedule.taskResult =  itemToSchedule._taskResult.promise;
        itemToSchedule.task = task;
    ...
    Scheduler.prototype.schedule = function (task,
                                             args,
                                             schedulerOverrides) {
        var workItem = this.wrapData(task, args, schedulerOverrides);
    ...
    Scheduler.prototype.runWork = function (workItem) {
        ...
        // TODO: task object is real, should eventually be a protocol object
        // to allow cross-process scheduler hosting
        workItem._taskResult = workItem.task.result;
        workItem.task.run.apply(workItem.task, workItem.args);

        // set up to resolve our outer promise if the task finishes first
        workItem._taskResult.then(function(value){
                return self.doneRunning(null, value, workItem);
            }, function(err){
                return self.doneRunning(err, null, workItem);
            });
